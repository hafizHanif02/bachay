                    ...
                    <div class="sub-card rounded-3 p-4">
                        <div class="card1">
                            <div class="first-sec card1">
                                ...
                                <div class="d-flex">
                                    <h4 class="card-text price">Rs. {{ $product->unit_price - ($product->unit_price * $product->discount) / 100 }}</h4>
                                    <p class="bg-primary rounded-pill ps-2 pe-2 ms-2 mt-1 text-white units">141 Solds</p>
                                </div>
                                <p class="card-text">
                                    <span class="discount">Rs. {{ $product->unit_price }}</span>
                                    <span class="text-success">-{{ $product->discount }}% Off</span>
                                </p>
                                <div class="subdiv d-flex justify-content-between">
                                    <a href="#">Standard Delivery</a>
                                    @foreach ($product->reviews as $review)
                                        <p class="rounded-pill text-white">{{ $review }} <img
                                                src="{{ asset('public/images/star.svg') }}" alt=""></p>
                                    @endforeach
                                    <h5>({{ $product->reviews_count }})</h5>
                                </div>
                            </div>
                        </div>
                    </div>
