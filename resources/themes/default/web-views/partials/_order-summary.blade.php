...
<aside class="col-lg-4 pt-4 pt-lg-2 px-max-md-0">
    <div class="__cart-total __cart-total_sticky">
        ...
        <form action="{{ 'checkout-payment' }}" id="order-form" method="POST">
            ...
            <input type="hidden" name="product[{{ $loop->iteration }}][tax]" value="{{ ($product['tax'] ?? '0') }}">
            ...
            <div class="mt-4">
                ...
                <button type="button" onclick="CheckoutFormSubmit()"  style="background: var( --greadient-normal, linear-gradient( 270deg, #845dc2 -0.09%, #d55fad 36.37%, #fc966c 72.82%, #f99327 100.48%, #ffc55d 145.17% ) ); border: 0px;" onclick="checkout()" class="col-12 btn btn--primary rounded-pill text-light btn-block proceed_to_next_button {{$cart->count() <= 0 ? '' : ''}}" >Place Order</button>
                @else
...
<div class="bottom-sticky3 bg-white p-3 shadow-sm w-100 d-lg-none">
    ...
    @if($web_config['guest_checkout_status'] || auth('customer')->check())
        <a onclick="checkout()" class="btn btn--primary btn-block proceed_to_next_button text-capitalize {{$cart->count() <= 0 ? '' : ''}}">{{translate('Place_order')}}</a>
    @else
        <a href="{{route('customer.auth.login')}}" class="btn btn--primary btn-block proceed_to_next_button text-capitalize {{$cart->count() <= 0 ? '' : ''}}">{{translate('Place_order')}}</a>
    @endif
</div>
@push('script')
    <script>
