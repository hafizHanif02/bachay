    ...
    <div class="container">
        <div class="login-card row justify-content-center">
            <div class="col-lg-6 shadow font-poppins p-4">
                <h2 class="text-center h4 mb-4 font-bold text-capitalize fs-18-mobile">{{ translate('login') }}</h2>
                <form class="needs-validation" autocomplete="off" action="{{ route('customer.auth.login') }}" method="post"
                    id="form-id">
                    @csrf
                    <div class="mb-3">
                        <label for="si-email" class="form-label font-semibold">Token</label>
                        <input type="text" class="form-control" name="token"
                            style="text-align: {{ Session::get('direction') === 'rtl' ? 'right' : 'left' }};"
                            value="{{ old('user_id') }}"
                            placeholder="{{ translate('enter_token_that_you_received_on_mail_or_whatsapp') }}" required>
                        {{-- <div class="invalid-feedback">{{ translate('please_provide_valid_email_or_phone_number') }}.</div> --}}
                    </div>
